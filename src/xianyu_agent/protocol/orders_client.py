...
def _safe_raw_payload(*, common: dict[str, Any], buyer: dict[str, Any], price: dict[str, Any]) -> dict[str, Any]:
    """只保留订单镜像所需字段, 避免把电话、收货地址等响应内容入库。"""
    return {
        "commonData": _pick(common, "orderId", "itemId", "orderStatus", "createTime", "inRefund"),
...
def _parse_order_time(value: Any) -> datetime | None:
    if not isinstance(value, str) or not value.strip():
        return None
    try:
        local = datetime.strptime(value.strip(), "%Y-%m-%d %H:%M:%S").replace(tzinfo=_ORDER_TIME_ZONE)
    except ValueError:
        return None
